...
from movies.models import Movie, Cast, Genre
...
class Command(BaseCommand):
    help = 'Load movies data into the database'
    ...
    def handle(self, *args, **kwargs):
        for movie_data in movies_data:
            movie, created = Movie.objects.get_or_create(
                ...
                extract=movie_data['extract'],
                thumbnail=movie_data['thumbnail'],
                thumbnail_width=movie_data['thumbnail_width'],
                thumbnail_height=movie_data['thumbnail_height']
            )
            
            for cast_name in movie_data['cast']:
                Cast.objects.get_or_create(movie=movie, name=cast_name)
                
            for genre_name in movie_data['genres']:
                Genre.objects.get_or_create(movie=movie, name=genre_name)
